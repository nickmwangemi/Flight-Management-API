...
from app.utils import validate_icao_code, parse_datetime, calculate_inflight_time
...
def get_aircraft():
	...
	return jsonify({
		'status': 'success',
		'data': [a.to_dict() for a in aircraft]
	}), 200
...
def create_flight():
	...
	# Validate required fields
	required_fields = ['departure_airport', 'arrival_airport', 'departure_time', 'arrival_time']
...
@api_bp.route('/flights/<int:flight_id>/assign-aircraft/<int:aircraft_id>', methods=['PUT'])
...
def flight_statistics():
	...
	for flight in flights:
		...
		if flight.aircraft_id:
			...
			stats[departure_airport]['aircraft_flight_times'][aircraft_id] += in_flight_minutes
	...
	for airport, data in stats.items():
		...
		if data['flight_count'] > 0:
			airport_data['average_flight_time'] = round(data['total_flight_time'] / data['flight_count'], 2)
