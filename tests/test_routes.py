...
class TestRoutes(unittest.TestCase):
	...
	def test_create_aircraft(self):
		...
		response = self.client.post('/api/aircraft',
	...
	def test_update_aircraft(self):
		...
		response = self.client.put(f'/api/aircraft/{self.aircraft1_id}',
		                           data=json.dumps(payload),
		                           content_type='application/json')
		data = json.loads(response.data)
	...
	def test_create_flight(self):
		...
		# Check it was actually added to the database
		flight = Flight.query.filter_by(departure_airport='EDDF', arrival_airport='LFPG').first()
	...
	def test_update_flight(self):
		...
		response = self.client.put(f'/api/flights/{self.flight1_id}',
		                           data=json.dumps(payload),
		                           content_type='application/json')
		data = json.loads(response.data)
	...
	def test_assign_aircraft_to_flight(self):
		response = self.client.put(f'/api/flights/{self.flight1_id}/assign-aircraft/{self.aircraft2_id}')
		data = json.loads(response.data)
	...
	def test_flight_statistics(self):
		...
		response = self.client.get(f'/api/reports/flight-stats?start_time={start_time}&end_time={end_time}')
		data = json.loads(response.data)
		...
		# Check results
		result = data['data']
		self.assertEqual(len(result), 2)  # Should have stats for two departure airports: ABCD and IJKL

		# Find ABCD airport stats
		abcd_stats = next((item for item in result if item['departure_airport'] == 'ABCD'), None)
		self.assertIsNotNone(abcd_stats)
		self.assertEqual(abcd_stats['flight_count'], 2)

		# Find IJKL airport stats
		ijkl_stats = next((item for item in result if item['departure_airport'] == 'IJKL'), None)
